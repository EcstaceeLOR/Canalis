...
function settlementTransaction(result: SettleResponse): string {
  ...
  if (!result.success || typeof transaction !== "string" || transaction.length === 0) {
    ...
    throw new Error(
      `Solana payment-channel settlement failed${typeof reason === "string" ? ` (${reason})` : ""}: ${
        typeof message === "string" ? message : "no confirmed transaction returned"
      }`,
    );
  }
  return transaction;
...
function payloadField(payload: PaymentPayload, key: string): string {
  ...
  if (typeof value !== "string" || value.length === 0) {
    throw new Error(`Payment payload is missing ${key}.`);
  }
  return value;
}

export function validateLivePaymentPayload(
  paymentPayload: PaymentPayload,
  requirements: PaymentRequirements,
  expectedPayer: string,
): void {
  if (paymentPayload.x402Version !== 2) throw new Error("Only x402 v2 live channels are supported.");
  if (payloadField(paymentPayload, "from") !== expectedPayer) {
    throw new Error("Payment payload payer does not match the authenticated wallet.");
  }
  const accepted = paymentPayload.accepted;
  for (const field of ["scheme", "network", "asset", "amount", "payTo"] as const) {
    if (accepted[field] !== requirements[field]) {
      throw new Error(`Payment payload accepted.${field} does not match the prepared channel.`);
    }
  }
  if (accepted.maxTimeoutSeconds !== requirements.maxTimeoutSeconds) {
    throw new Error("Payment payload timeout does not match the prepared channel.");
  }
}
...
export class DevnetX402ChannelGateway {
  ...
  private readonly serverScheme: UptoSvmServerScheme;
    ...
    if (!this.ready) {
      this.ready = (async () => {
        ...
        if (genesis !== SOLANA_DEVNET_GENESIS_HASH) {
          throw new Error(
            `Refusing to use Canalis public sandbox identities outside Solana devnet. Expected genesis ${SOLANA_DEVNET_GENESIS_HASH}, got ${genesis}.`,
          );
        }
        ...
        if (sponsorLamports < MIN_SPONSOR_LAMPORTS) {
          throw new Error(
            `Canalis devnet sandbox sponsor ${this.sponsor.publicKey.toBase58()} needs devnet SOL before live channels can be opened.`,
          );
        }
        const mint = await getMint(
          this.connection,
          new PublicKey(CANALIS_DEVNET_SANDBOX_MINT),
          "confirmed",
          TOKEN_PROGRAM_ID,
        );
        if (mint.decimals !== 6 || mint.mintAuthority?.toBase58() !== this.sponsor.publicKey.toBase58()) {
          throw new Error("Configured Canalis devnet sandbox mint failed authority/decimals verification.");
        }
        await getOrCreateAssociatedTokenAccount(
          this.connection,
          this.sponsor,
          new PublicKey(CANALIS_DEVNET_SANDBOX_MINT),
          new PublicKey(CANALIS_DEVNET_TREASURY_OWNER),
          true,
          "confirmed",
          undefined,
          TOKEN_PROGRAM_ID,
        ...
        this.facilitator.register(
          ...
          new UptoSvmFacilitatorScheme(
            ...
            { maxChannelLifetimeSecs: 3_600, authorizerSigner },
          ),
        );
        this.serverScheme = new UptoSvmServerScheme({
  ...
  async fundSandboxWallet(walletAddress: string, requiredAtomic: bigint): Promise<string> {
    ...
    if (requiredAtomic <= 0n || requiredAtomic > CANALIS_LIVE_SANDBOX_MAX_TASK_ATOMIC) {
      throw new Error(
        `Live devnet sandbox funding must be between 1 and ${CANALIS_LIVE_SANDBOX_MAX_TASK_ATOMIC} atomic units.`,
      );
    }
  ...
  async prepareChannel(input: {
    taskId: string;
    payer: string;
    providerId: string;
    ceilingAtomic: bigint;
    taskExpiresAtUnixSeconds: bigint;
  }): Promise<LiveChannelPreparation> {
    await this.initialize();
    ...
    const requirements: PaymentRequirements = {
      scheme: "upto",
      network: NETWORK,
      asset: CANALIS_DEVNET_SANDBOX_MINT,
      amount: input.ceilingAtomic.toString(),
      payTo: providerPayTo,
      maxTimeoutSeconds,
      extra: {
        feePayer: facilitator,
        receiverAuthorizer: receiver,
        withdrawDelay: Math.max(300, maxTimeoutSeconds),
        tokenProgram: TOKEN_PROGRAM_ID.toBase58(),
        memo: `canalis:${input.taskId}:${providerId}`,
      },
    };
    return {
      providerId,
      paymentRequired: {
        x402Version: 2,
        resource: {
          url: `canalis://tasks/${input.taskId}/providers/${providerId}`,
          description: `Canalis live devnet channel for ${providerId}`,
          mimeType: "application/json",
          serviceName: "Canalis",
          tags: ["solana", "payment-channel", "devnet"],
        },
  ...
  async deposit(
    ...
    const result = await this.settleWithSafeRetry(
      { ...paymentPayload, payload: { ...paymentPayload.payload, type: "deposit" } },
      requirements,
    );
    return {
      channelId,
      transactionSignature: settlementTransaction(result),
  ...
  async claim(
    ...
    const claimRequirements: PaymentRequirements = {
      ...requirements,
      amount: settledAtomic.toString(),
    };
    const enrichment = await this.serverScheme.enrichSettlementPayload({
      paymentPayload,
    ...
    const claimPayload: PaymentPayload = {
      ...paymentPayload,
      payload: { ...paymentPayload.payload, ...(enrichment ?? { type: "claim" }) },
    };
    const result = await this.settleWithSafeRetry(claimPayload, claimRequirements);
    return {
      channelId: payloadField(paymentPayload, "channelId"),
      transactionSignature: settlementTransaction(result),
      settledAtomic: accounting.settledAtomic.toString(),
      refundedAtomic: accounting.refundedAtomic.toString(),
    };
  }
}
