        ...
        if (action === "submit" || action === "cancel" || action === "archive") {
          const allowed = action === "submit"
            ...
            : action === "cancel"
              ...
              : ["draft", "completed", "cancelled"] as const;
          ...
          const changed = await repository.setStatus(
            id,
            identity.walletAddress,
            nextStatus,
            now,
          ...
          if (!changed) {
            throw new ApplicationError(
              "INVALID_TASK_TRANSITION",
              "The task changed while this request was in progress. Refresh it before retrying.",
              409,
            );
          }
