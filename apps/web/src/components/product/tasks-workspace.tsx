...
export function TasksWorkspace() {
  ...
  }, [loadProviders, queryString, session]);
    ...
    <div className="tasks-workspace">
      <div className="tasks-toolbar">
        <div className="tasks-search">
          <span>⌕</span>
          <input aria-label="Search tasks" placeholder="Search name, description, task ID…" value={search} onChange={(event) => setSearch(event.target.value)} />
        </div>
        <select aria-label="Filter by status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
          <option value="all">All statuses</option><option value="draft">Draft</option><option value="active">Active</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option><option value="archived">Archived</option>
        </select>
        ...
        <select aria-label="Filter by creation date" value={range} onChange={(event) => setRange(event.target.value)}>
          <option value="all">Any date</option><option value="24h">Last 24h</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option>
        </select>
        <select aria-label="Sort tasks" value={sort} onChange={(event) => setSort(event.target.value)}>
          <option value="updated_desc">Recently active</option><option value="created_desc">Newest</option><option value="created_asc">Oldest</option><option value="name_asc">Name A–Z</option>
        </select>
        ...
        <button className="primary-action" type="button" onClick={openCreate}>New task <span>＋</span></button>
      </div>
      ...
      {loading && result.tasks.length === 0 ? (
        ...
        <section className="tasks-empty">
          <div className="empty-symbol">＋</div>
          <div><h2>{search || statusFilter !== "all" || provider !== "all" || range !== "all" ? "No tasks match these filters" : "Create your first governed task"}</h2><p>{search || statusFilter !== "all" || provider !== "all" || range !== "all" ? "Change or clear the workspace filters to broaden the result set." : "Set a bounded budget, choose verified providers, save a draft or create an active task. It will remain here across refreshes and deployments."}</p></div>
          <button type="button" onClick={openCreate}>Create task</button>
        </section>
      ) : view === "table" ? (
        <div className="tasks-table-wrap"><table className="tasks-table"><thead><tr><th>Task</th><th>Status</th><th>Budget</th><th>Spent</th><th>Recoverable</th><th>Providers</th><th>Owner</th><th>Created</th><th>Last activity</th><th /></tr></thead><tbody>{result.tasks.map((task) => <TaskRow key={task.id} task={task} busy={busyId === task.id} lifecycle={lifecycle} />)}</tbody></table></div>
      ) : (
      ...
      {createOpen ? <TaskComposer form={form} setForm={setForm} providers={providers} toggleProvider={toggleProvider} changeMode={changeMode} creating={creating} close={() => setCreateOpen(false)} createTask={createTask} /> : null}
    </div>
  );
}
  ...
  return (
    <div className="task-modal-backdrop" role="presentation" onMouseDown={close}>
      <div className="task-composer" role="dialog" aria-modal="true" aria-label="Create Canalis task" onMouseDown={(event) => event.stopPropagation()}>
        <div className="task-composer-head"><div><span>New autonomous payment task</span><h2>Define the task envelope</h2><p>The wallet owns the task. Canalis enforces budget, provider health, protocol compatibility and spend limits server-side.</p></div><button type="button" onClick={close}>×</button></div>
