...
const emptyResult: Result = {
  records: [],
  summary: { authorizationCount: 0, authorizationAtomic: "0", settledAtomic: "0", recoveredAtomic: "0", failedCount: 0, onchainCount: 0 },
  total: 0,
  page: 1,
  pageSize: 25,
  totalPages: 1,
...
function explorer(signature?: string, network?: string) {
  ...
  const cluster = network?.toLowerCase().includes("mainnet") ? "" : "?cluster=devnet";
...
function kindLabel(kind: TransactionRecord["kind"]) {
  return kind === "channel_open" ? "Channel open" : kind[0].toUpperCase() + kind.slice(1);
}
...
export function TransactionsWorkspace() {
  ...
  }, [kind, network, page, protocol, provider, range, search, sort, status, task]);
  ...
  const loadLookups = useCallback(async () => {
    ...
    if (providerResponse.ok) {
      const body = (await providerResponse.json()) as { providers: Provider[] };
      setProviders(body.providers);
    }
    if (taskResponse.ok) {
      const body = (await taskResponse.json()) as { tasks: Task[] };
      setTasks(body.tasks);
    }
  }, [session]);
  ...
  function exportUrl(format: "csv" | "json") {
    ...
    return `/api/transactions/export?${query.toString()}`;
  }

  if (!session) {
    return <section className={styles.connect}><div>≡</div><section><h2>Connect your wallet to open the audit explorer</h2><p>{walletStatus === "loading" ? "Checking your wallet session…" : "Transactions and receipts are scoped to the authenticated wallet and its tasks."}</p></section></section>;
  }
  ...
  return <div className={styles.workspace}>
    ...
    <section className={styles.toolbar}>
      ...
      <select aria-label="Filter network" value={network} onChange={(event) => setNetwork(event.target.value)}><option value="all">All networks</option>{networks.map((item) => <option value={item} key={item}>{item}</option>)}</select>
      <select aria-label="Filter status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option><option value="confirmed">Confirmed</option><option value="fulfilled">Fulfilled</option><option value="rejected">Rejected</option><option value="failed">Failed</option></select>
      <select aria-label="Filter date range" value={range} onChange={(event) => setRange(event.target.value)}><option value="all">Any date</option><option value="24h">Last 24h</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select>
    ...
    <section className={styles.listHead}><div><strong>{result.total}</strong><span> audit records</span></div>{loading ? <span>Refreshing…</span> : null}</section>
